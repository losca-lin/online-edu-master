...
public class TOrderService {
	...
	public Integer updateById(TOrderEntity order){
		return tOrderMapper.updateById(order);
	}
	public TOrderEntity selectByOrdeoNo(String OrdeoNo){
		QueryWrapper<TOrderEntity> wrapper = new QueryWrapper<>();
		wrapper.eq("order_no", OrdeoNo);
	...
	public R paySucceed(String orderNo) {
		...
		EduCourseEntity educourse = eduCourseMapper.selectById(entity.getCourseId());
		Integer buyCount = educourse.getBuyCount();
