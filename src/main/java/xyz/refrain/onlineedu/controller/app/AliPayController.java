...
import xyz.refrain.onlineedu.config.AlipayConfig;
    ...
    public String payNotify(HttpServletRequest request) throws Exception {
        if (request.getParameter("trade_status").equals("TRADE_SUCCESS")) {
            ...
            if (checkSignature) {
                ...
                // // 查询订单
                TOrderEntity order = tOrderService.selectByOrdeoNo(outTradeNo);

                if (order != null) {
                    order.setTransactionNum(alipayTradeNo);
                    order.setPayTime(LocalDateTime.now());
                    order.setPayType(PayTypeEnum.ALI_PAY);

                    tOrderService.updateById(order);
                    R r = tOrderService.paySucceed(outTradeNo);
